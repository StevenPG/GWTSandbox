...
public class MemoryGameTextDriver {
	...
	/**
	 * Internal object that keeps track of necessary data for access and
	 * mutation during the runtime of the application.
	 */
	private MemoryGameState state;

	/**
	 * General constructor doesn't do anything useful except say that the game
	 * has not started yet, instantiate a writer object, and generate the game
	 * state.
	 */
	public MemoryGameTextDriver(int row, int column) {
		...
		this.row = row;
		this.column = column;
		this.state = new MemoryGameState();
	}
	...
	public void playTurn() {
		...
		try {
			...
			// If cards are a match, print success message and lock as pair
			// linked
			...
			System.out
					.println("Please never do what you just did, EVER again.");
			System.out.println(
					"Since you might be Dr. Spiegel testing this code,");
			System.out
					.println("I guess I'll just start another turn for you.\n");
