...
public class ComputerPlayer extends MemoryPlayer implements Player {
     ...
     * @param compareCard The card to check
    ...
    public MemoryCard getSecondChoice(MemoryGameBoard board) {
        ...
        if (difficultyLevel == 0) {
            ...
            MemoryCard secondCard = board.getCard(secondNumber);
            while (!isCardValid(secondCard)) {
                secondNumber = random.nextInt(30);
                secondCard = board.getCard(secondNumber);
            }
            return secondCard;
        } else {
            // TODO
