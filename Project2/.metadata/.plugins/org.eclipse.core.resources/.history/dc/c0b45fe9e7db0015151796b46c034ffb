...
Design Decisions:
	I decided that I would write the code in an object oriented way such 
	that when moving from a text based game design to an visual interface 
	based design, I wouldn't have to change code. Namely I wouldn't have 
	to make any alterations to the CardFace interface and the MemoryCard,
	MemoryGameBoard, MemoryGameState, and SpiegelCode classes. The only
	things that will need to be added will be a new CardFace subclass that
	uses images rather than strings, and a new driver that manages the program
	using the MemoryGameBoard, MemoryGameState, and SpiegelCode classes.
